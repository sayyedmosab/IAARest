...
import { Component, ChangeDetectionStrategy, inject, signal, OnInit, OnDestroy, AfterViewInit, ViewChild, ElementRef } from '@angular/core';
...
export class ArCheckoutComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  async initializeStripeElements() {
    ...
    try {
      ...
      this.paymentError.set('فشل في تهيئة نموذج الدفع');
    }
  }
  ...
  private async processCreditCardPayment(user: User, plan: Plan) {
    if (!this.stripeElements) {
      this.paymentError.set('نموذج الدفع غير مهيأ');
      return;
    }
      ...
      if (subscriptionResult?.success && subscriptionResult.data) {
        ...
        if (paymentResult?.success && paymentResult.data) {
          ...
          if (confirmResult.success) {
            ...
            this.paymentError.set(confirmResult.error || 'فشل الدفع');
          }
        } else {
          this.paymentError.set(paymentResult?.error || 'فشل في إنشاء الدفع');
        }
      } else {
        this.paymentError.set('فشل في إنشاء الاشتراك');
      }
    } catch (error) {
      console.error('Payment processing error:', error);
      this.paymentError.set('حدث خطأ غير متوقع أثناء معالجة الدفع');
    } finally {
